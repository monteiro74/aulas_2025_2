...
| Programação                 | Assemblies                                        | Integração com .NET           | Permitem rodar código CLR (C#, VB.NET) dentro do SQL Server.                        |
| Programação                 | Tipos                                             | Estruturas personalizadas     | Tipos de dados definidos pelo usuário (UDTs) e tipos de tabela.                     |
| Programação                 | Regras                                            | Restrições antigas            | Definem regras de validação, hoje substituídas por CHECK constraints.               |
...
| Repositório de Consultas    | Consultas com Planos Forçados                     | Controle de execução          | Mostra queries executadas com planos de execução forçados manualmente.              |
...
| **7. 🏷️ Service Broker**              |                                                   | Mensageria assíncrona         | Gerencia troca de mensagens entre aplicações e bancos.                              |
...
| Service Broker              | Serviços                                          | Pontos de comunicação         | Definem a lógica que processa mensagens de filas.                                   |
...
| **9. 👮‍♂️ Segurança**                   |                                                   | Controle de acesso            | Gerencia autenticação, permissões e criptografia.                                   |
...
| Segurança                   | Chaves Assimétricas                               | Criptografia                  | Usadas em assinaturas digitais e criptografia com chave pública/privada.            |
| Segurança                   | Certificados                                      | Autenticação                  | Fornecem autenticação e podem proteger dados ou conexões.                           |
| Segurança                   | Chaves Simétricas                                 | Criptografia                  | Usam a mesma chave para criptografar e descriptografar dados.                       |
| Segurança                   | Chaves Always Encrypted                           | Proteção em trânsito          | Mantêm dados criptografados em colunas sensíveis, acessíveis apenas pelo cliente.   |
| Segurança                   | Especificações de Auditoria de Bancos de Dados    | Auditoria                     | Registram acessos e alterações para fins de conformidade.                           |
| Segurança                   | Políticas de Segurança                            | Regras avançadas              | Incluem Row-Level Security e Dynamic Data Masking para controle de acesso granular. |
